...
DROP FUNCTION IF EXISTS test.lire_dernier_jour_du_mois( date ) CASCADE;
CREATE OR REPLACE FUNCTION test.lire_dernier_jour_du_mois( d date ) 
RETURNS TEXT AS $$
...
DROP FUNCTION IF EXISTS test.num_to_words_lang( numeric, TEXT, text ) CASCADE;
CREATE OR REPLACE FUNCTION test.num_to_words_lang( numeric, TEXT, text DEFAULT 'currency' )
RETURNS text AS $$
    from num2words import num2words
    return num2words(args[0], lang=args[1], to=args[2])
$$ LANGUAGE plpython3u SET search_path TO test security DEFINER;
 -- SELECT test.num_to_words_lang( 2024, 'fr', 'currency' );


DROP FUNCTION IF EXISTS test.quittance_remplir( TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT ) CASCADE;
CREATE OR REPLACE FUNCTION test.quittance_remplir( quittance_fmt TEXT, bailleur_denomination TEXT, adresse_bailleur TEXT, locataire_denomination TEXT, adresse_locataire TEXT, date_debut_quittance TEXT, 
...
DROP FUNCTION IF EXISTS test.calcul_nb_jours_prorata( date, date ) CASCADE;
CREATE OR REPLACE FUNCTION test.calcul_nb_jours_prorata( fin date, deb date )
RETURNS int AS $$
	declare 
		nb_jours int;
	BEGIN
...
$$
LANGUAGE plpgsql SET search_path TO test security DEFINER;
COMMENT ON FUNCTION test.calcul_nb_jours_prorata IS 'HELPER FUNCTION. Renvoi le nombre de jours ''consommés'' dans le mois. Utile pour les mois d''arrivée et de sortie. En cas d''entrée le 01 et/ou de sortie le 30/31 ( dernier jour du mois ), force un nb jour mini à 1 au lieu de 0.';


DROP FUNCTION IF EXISTS test.quittance_periode_arret( int ) CASCADE;
CREATE OR REPLACE FUNCTION test.quittance_periode_arret( locataire_id int )
RETURNS text AS $$
	declare 
		date_fin_bail date;
		d date;
...
$$
LANGUAGE plpgsql SET search_path TO test security DEFINER;
COMMENT ON FUNCTION test.quittance_periode_arret IS 'HELPER FUNCTION. Renvoi la période d''arret des quittances. Si fin de bail connu : la période d''arrêt est la date de fin de bail. Sinon, on s''arrete à la période courrante';


-- SELECT test.quittance_peeriode_arret( 3 );
